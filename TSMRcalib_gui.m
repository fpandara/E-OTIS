...
function params=TSMRcalib_gui(cData,outloc,dx,dt,T,M,V,tin,MI,algC,alg1,alg2,trt,datt)
% clear all;clc;
A =cData(1,1);
As=cData(2,1);
D=cData(3,1);
...
LB=[cData(:,2);alg1];
UB=[cData(:,3);alg2];
params=fminsearchbnd(@TSMandR_calib,beta0,LB,UB,options,outloc,dx,dt,T,M,V,tin,RD,OD,PD,trt,datt);
